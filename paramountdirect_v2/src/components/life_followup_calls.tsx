...
export default function LifeFollowupCalls() {
  ...
  return (
    <div className="p-4 md:p-8 max-w-[1400px] mx-auto font-sans text-slate-900 space-y-6">
      <div>
        <h1 className="text-xl font-black uppercase tracking-widest text-slate-900 font-['Montserrat']">FOLLOW-UP CALLS</h1>
        <p className="text-xs font-semibold text-slate-500 mt-1">{successRatePct}% successful follow-up calls, for the past month</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm">
            <h2 className="text-xs font-extrabold text-emerald-600 uppercase tracking-wide mb-3">Successful</h2>
            <div className="flex items-baseline space-x-3 mb-2">
              ...
              <span className="text-sm font-bold text-slate-400">{current.successful - previous.successful >= 0 ? '+' : ''}{current.successful - previous.successful}</span>
            </div>
            <p className="text-xs font-semibold text-slate-600">{current.successful} / {current.total} of total</p>
            <p className="text-xs font-semibold text-slate-600">{avgSuccessfulPerDay} average per day</p>
          </div>

          <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm">
            <h2 className="text-xs font-extrabold text-rose-600 uppercase tracking-wide mb-3">Unsuccessful</h2>
            <div className="flex items-baseline space-x-3 mb-2">
              <span className="text-2xl font-black text-rose-600 flex items-center">
                {unsuccessful} <TrendingDown className="w-4 h-4 ml-1" />
              </span>
              <span className="text-sm font-bold text-slate-400">{unsuccessful - unsuccessfulPrev >= 0 ? '+' : ''}{unsuccessful - unsuccessfulPrev}</span>
            </div>
            <p className="text-xs font-semibold text-slate-600">{unsuccessful} / {current.total} of total</p>
            <p className="text-xs font-semibold text-slate-600">{avgUnsuccessfulPerDay} average per day</p>
          </div>
        </div>

        {/* Right: chart */}
        <div className="lg:col-span-8 bg-white rounded-3xl border border-slate-200 p-6 shadow-sm flex flex-col">
          <h2 className="text-sm font-extrabold text-slate-800 uppercase mb-6">Monthly Trend</h2>
          ...
          <div className="flex flex-wrap items-center gap-3 mt-6 pt-4 border-t border-slate-100 text-[10px] font-bold text-slate-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {TIERS.map((t) => (
          <div key={t.tier} className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm text-center">
            <h3 className="text-xs font-extrabold text-slate-500 uppercase tracking-wide mb-4">{t.tier}</h3>
            <div className="flex items-center justify-center space-x-8">
              ...
              <div>
                <p className="text-2xl font-black text-amber-500">{t.pending}</p>
      ...
      <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm">
        <h2 className="text-sm font-extrabold text-slate-800 uppercase mb-1">Pending Follow-ups</h2>
        <p className="text-xs text-slate-500 font-medium mb-5">Applications due for a follow-up call today</p>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-100">
              {pending.map((call) => (
                <tr key={call.id} className={call.isCritical ? 'bg-rose-50/40' : ''}>
                  <td className="py-3.5 px-2 font-bold text-slate-900">{call.id}</td>
                  <td className="py-3.5 px-2 font-bold text-slate-900">{call.payor}</td>
                  <td className="py-3.5 px-2">
                    <span className="inline-flex items-center space-x-1.5 px-2.5 py-1 rounded-full text-[10px] font-extrabold bg-slate-100 text-slate-700 border border-slate-200">
                      {call.isCritical && <ShieldAlert className="w-3 h-3 text-rose-500" />}
                      <span>{call.tier}</span>
                    </span>
                  </td>
                  <td className="py-3.5 px-2 font-semibold text-slate-700">{call.mobile}</td>
                  <td className="py-3.5 px-2 font-semibold text-slate-700">{call.dueDate}</td>
