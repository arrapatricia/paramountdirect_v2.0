...
export default function CtplApplicationList({ data, onCreateNew, viewingId = null, onView, onCloseView, connected = false, onPolicyChanged }: Props) {
  ...
  return (
    <div className="p-4 md:p-8 space-y-6 max-w-[1650px] mx-auto font-sans text-slate-900 dark:text-slate-100">
      ...
      {/* Detail View Modal - a paid application is a read-only quick preview,
          so it's a prompt/overlay rather than its own routed page (unlike
          the unpaid ctpl_application_detail.tsx page, which still supports
          editing). Matches ofw_application_list.tsx's paid-view modal. */}
      {viewingApp && (
