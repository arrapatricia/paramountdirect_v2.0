...
export default function LifeScreenedApplications() {
  return (
    <div className="p-4 md:p-8 max-w-[1400px] mx-auto font-sans text-slate-900 space-y-6">
      <h1 className="text-xl font-black uppercase tracking-widest text-slate-900 font-['Montserrat']">SCREENED APPLICATIONS</h1>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        {/* Left stats */}
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm">
            <h2 className="text-xs font-extrabold text-slate-500 uppercase tracking-wide mb-2">Applications</h2>
            <p className="text-2xl font-black text-[#d0112b]">{totalPast3Months.toLocaleString()}</p>
            <p className="text-xs font-semibold text-slate-500">for the past 3 months</p>
          </div>

          <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm">
            <h2 className="text-xs font-extrabold text-slate-500 uppercase tracking-wide mb-3">Per Day</h2>
        ...
        <div className="lg:col-span-8 bg-white rounded-3xl border border-slate-200 p-6 shadow-sm flex flex-col">
          <h2 className="text-sm font-extrabold text-slate-800 uppercase mb-6">Daily Trend</h2>
      ...
      {/* Screener breakdown */}
      <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm">
        <h2 className="text-sm font-extrabold text-slate-800 uppercase mb-1">Screener Breakdown</h2>
        <p className="text-xs text-slate-500 font-medium mb-5">Applications screened by each issuer this month</p>
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-100">
              {SCREENER_BREAKDOWN.map((s) => (
                <tr key={s.screener}>
                  <td className="py-3.5 px-2 font-bold text-slate-900">{s.screener}</td>
                  <td className="py-3.5 px-2 text-right font-black text-slate-800">{s.screenedThisMonth.toLocaleString()}</td>
