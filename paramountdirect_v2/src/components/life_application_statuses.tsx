...
export default function LifeApplicationStatuses() {
  return (
    <div className="p-4 md:p-8 max-w-[1400px] mx-auto font-sans text-slate-900 space-y-6">
      <h1 className="text-xl font-black uppercase tracking-widest text-slate-900 font-['Montserrat']">APPLICATION STATUSES</h1>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        {/* Chart */}
        <div className="lg:col-span-8 bg-white rounded-3xl border border-slate-200 p-6 shadow-sm flex flex-col">
          <h2 className="text-sm font-extrabold text-slate-800 uppercase mb-1">Monthly Breakdown by Status</h2>
          <p className="text-xs text-slate-500 font-medium mb-6">Grouped bars per status, 2026</p>
        ...
        <div className="lg:col-span-4 bg-white rounded-3xl border border-slate-200 p-6 shadow-sm">
          <h2 className="text-sm font-extrabold text-slate-800 uppercase mb-1">YTD Breakdown</h2>
          <p className="text-xs text-slate-500 font-medium mb-5">{grandTotal.toLocaleString()} applications, Jan&ndash;Sep 2026</p>
            ...
            {ytdTotals.map((s) => (
              <div key={s.key}>
                <div className="flex items-center justify-between text-xs mb-1">
                  ...
                  <span className="font-black text-slate-900">{s.total.toLocaleString()} <span className="text-slate-400 font-semibold">({((s.total / grandTotal) * 100).toFixed(0)}%)</span></span>
                </div>
