...
export default function Login({ onLoginSuccess, darkMode, setDarkMode }: LoginProps) {
  ...
  return (
    <div className={`relative min-h-screen flex items-center justify-center p-4 overflow-hidden font-sans transition-colors duration-300 ${
      darkMode 
      ...
      <button
        ...
        className={`absolute top-6 right-6 p-3 rounded-2xl backdrop-blur-md border cursor-pointer shadow-md z-20 transition-all ${
          darkMode
      ...
      {/* Restored Liquid Glass Orbs */}
      <div className={`absolute -top-20 -left-20 w-96 h-96 rounded-full blur-3xl pointer-events-none animate-pulse z-10 ${
        darkMode ? 'bg-[#d0112b]/30' : 'bg-[#d0112b]/25'
      }`} />
      <div className={`absolute -bottom-20 -right-20 w-96 h-96 rounded-full blur-3xl pointer-events-none z-10 ${
        darkMode ? 'bg-red-600/20' : 'bg-[#d0112b]/15'
      }`} />

      {/* Main Login Card (Liquid Glass Aesthetics) */}
      <div className={`relative z-20 w-full max-w-md p-8 md:p-10 rounded-3xl backdrop-blur-2xl border transition-all ${
        darkMode
          ? 'bg-gray-900/50 border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.6)]'
          : 'bg-white/75 border-white/90 shadow-[0_20px_50px_rgba(208,17,43,0.15)]'
      }`}>
        
        {/* Header Section (Clean Logo) */}
        <div className="text-center mb-6">
          <div className="flex justify-center mb-2">
            <img 
              src={darkMode ? pdLogoWhite : pdLogoFullColor} 
              alt="Paramount Direct Logo" 
              className="h-24 w-auto object-contain filter drop-shadow-[1px_2px_4px_rgba(0,0,0,0.2)]"
            />
          </div>
        </div>
        ...
        {errorMessage && (
          <div className="mb-5 p-3.5 rounded-2xl bg-red-500/20 border border-red-500/40 text-red-600 dark:text-red-300 flex items-center space-x-2.5 text-xs font-medium backdrop-blur-md">
        ...
        <form className="space-y-5" onSubmit={handleSubmit}>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className={`block w-full pl-10 pr-4 py-3 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-[#d0112b] focus:border-transparent backdrop-blur-md ${
                  darkMode
          ...
          <div>
            ...
            <div className="relative">
              <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                <Lock className={`h-4 w-4 ${darkMode ? 'text-gray-400' : 'text-gray-600'}`} />
              </div>
              <input
                type="password"
                required
          ...
          <div className="flex items-center justify-between text-xs pt-1">
            <label className={`flex items-center cursor-pointer ${
              ...
              <input
                type="checkbox"
                className="h-4 w-4 rounded-md border-gray-300 text-[#d0112b] focus:ring-[#d0112b] focus:ring-offset-0"
              />
              <span className="ml-2 font-medium">Remember me</span>
            </label>

            <a href="#" className="font-semibold text-[#d0112b] hover:text-[#b00e24]">
              Forgot password?
            </a>
          </div>
          ...
          <button
            type="submit"
            className="w-full py-3 px-4 rounded-2xl font-semibold text-sm text-white bg-[#d0112b] hover:bg-[#b00e24] focus:outline-none focus:ring-2 focus:ring-[#d0112b] focus:ring-offset-2 shadow-[0_8px_20px_rgba(208,17,43,0.35)] hover:shadow-[0_10px_25px_rgba(208,17,43,0.5)] transform active:scale-[0.98] cursor-pointer transition-all"
          >
            Sign In
          </button>
        </form>
      ...
      <div className={`absolute bottom-6 right-6 text-xs font-mono px-2.5 py-1 rounded-lg border backdrop-blur-md z-20 shadow-sm ${
        darkMode 
