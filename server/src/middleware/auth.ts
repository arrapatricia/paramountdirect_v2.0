...
// For server-to-server ingestion (the public paramountdirect.com website
// pushing a submitted application here) rather than a logged-in admin user -
// a static shared key instead of a JWT, since there's no user session on the
// other end to issue one to. Populates req.user with a synthetic identity so
...
export function requireServiceApiKey(req: Request, res: Response, next: NextFunction) {
  const expected = process.env.WEBSITE_INGEST_API_KEY;
  if (!expected) {
    return res.status(503).json({ error: 'Website ingest is not configured (WEBSITE_INGEST_API_KEY unset)' });
  }
  ...
  req.user = { email: 'paramountdirect.com (website)', role: 'System', assignedProducts: ['PD Life'] };
  next();
}
