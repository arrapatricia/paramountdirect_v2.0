...
**Prepared:** 2026-09-15 · **Last updated:** 2026-09-21 (OFW/CTPL/GTP wired to real backend persistence + website ingest webhooks, matching PD Life's existing pattern)
...
| CTPL | `ctpl_dashboard.tsx`, `ctpl_application_list.tsx`, `ctpl_create_application.tsx`, `ctpl_types.ts` | Real rate table by Policy Type × MV Type. Straight-through payment (no verification gate) before documents unlock — `isPaid` is set true at creation itself, unlike OFW. Wired to real backend persistence (`/api/applications/ctpl`) and dashboard stats computed live from that data. |
