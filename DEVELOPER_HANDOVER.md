...
**Prepared:** 2026-09-15 · **Last updated:** 2026-09-25 (OFW's Service Invoice is now real too, tax breakdown verified against the legacy Rails source - see §1 and §3)
...
Wiring OFW to real persistence required extending the Prisma schema (migration `20260921132500_add_ofw_ctpl_gtp_missing_fields`, applied to the shared `paramountdirectdev-v2-db` dev database): `OfwApplication` gained `phRegion`/`phBarangay`/`employmentVerified`/`paymentInstructionSent`/`isPaid` plus a new `OfwBeneficiary` child table; `CtplApplication` gained the owner-address breakdown fields plus `isPaid`; `GtpApplication` gained `isPaid`. See `DATABASE_SCHEMA.md` (and its live artifact) for the current field list. As of migration `20260925080000_rename_tables_snake_case`, every table is `@@map`ped to a snake_case plural name (`OfwApplication` → `ofw_applications`); Prisma model names in code are unchanged, but raw SQL must use the table name (see `ipeak/policyNumber.ts`).
...
| OFW | `ofw_dashboard.tsx`, `ofw_application_list.tsx`, `ofw_create_application.tsx`, `ofw_types.ts` | Real rate card: $0.0954/day × contract months (No. of Months auto-computed, 6-month minimum enforced). Generated documents (below) gated behind an Employment Contract Verification step (Yes/No) → Send Payment Instruction → payment confirmation. Wired to real backend persistence (`/api/applications/ofw`) and dashboard stats computed live from that data. The server stamps `paymentInstructionSentBy` (the logged-in user's name) once, the first time the payment instruction is sent, for tracking (migration `20260925060000_add_ofw_payment_instruction_sent_by`). It shows in the view modal next to Payment Status (Paid/Unpaid) and Policy/Application Status. |
...
| Generated policy documents | `policy_documents.tsx` (shared modal chrome, used only by OFW's still-mock COI section and GTP's document section); CTPL and OFW's Service Invoice instead call the real `/api/documents` API (§1) | **CTPL:** Certificate of Cover + Service Invoice are real generated PDFs. **OFW:** Service Invoice is now real too (`ofwDocumentFill.ts`) — its tax breakdown (flat $0.600 Documentary Stamp + computed LGT/Other Fees, entirely VAT-exempt) was reverse-engineered from and verified against the actual legacy Rails source (`github.com/plgic/paramountdirect`'s `Ofw::Application#life_premium`/`#non_life_premium`/`#premium_tax`/`#lg_tax`, `PaymentTransaction#invoice_data`'s `'ofw'` branch) and matches a real sample invoice to the thousandth. OFW's **Certificate of Insurance (COI)** is still the old mock-template render — Bernadette's Drive folder only had *static* (flattened, 0-field) sample PDFs for BM/DH, not real fillable templates, so there's nothing to fill yet; the CTPL bug this same investigation caught (Unit/Total Cost were showing the base premium, not the invoice grand total) is now fixed too. **GTP:** still the older mock-template render, no real GTP templates on hand yet. |
...
| Branch Directory | `branch_directory.tsx` | Mock CRUD. Seeded with the 18 branches on ofwinsurance.ph/ofw-branches (the source of truth, all Non-life). Has a "Branch Locator" view mirroring that page (region picker + expandable regions), the default for OFW/CTPL/GTP; PD Life defaults to Grid Cards |
| Marketing Dashboard | `marketing_dashboard.tsx` | GA/source/drop-off stats zeroed (no mock analytics); the fabricated "unfinished applications" rows were removed entirely (empty-state table). With OFW selected it shows ofwinsurance.ph pages and the OFW form's sections instead of paramountdirect.com's |
...
| SI & Official Receipt Issuance | High | 🟡 | **Service Invoice is now real** — a genuine generated, S3-stored PDF with a verified tax breakdown (§1/§3), tied to `isPaid`. **OR is still mock** (no real fillable OR template on hand) |
